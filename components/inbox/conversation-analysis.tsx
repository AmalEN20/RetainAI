...
export function ConversationAnalysis({ conversationId = 1, customerId = "cus_acme_001", customer = "Acme Inc.", initials = "AC", contact = "Sarah Chen" }: { conversationId?: number; customerId?: string; customer?: string; initials?: string; contact?: string }) {
  ...
  if (status === "idle") {
    return (
      <div className="rounded-xl border border-[#cfe6da] bg-[#f1f9f5] p-4 sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <Button className="w-full sm:w-auto" data-demo-target="analyze-message" onClick={runAnalysis}><Sparkles className="h-4 w-4" /> Analyze message</Button>
        </div>
  ...
  if (status === "error") {
    return (
      <div className="rounded-xl border border-[#f0d1cb] bg-[#fff5f3] p-5">
        <div className="flex flex-wrap items-start gap-3"><XCircle className="mt-0.5 h-5 w-5 text-[#b74b3a]" /><div className="min-w-0 flex-1"><h3 className="text-sm font-bold text-[#7f3025]">Analysis interrupted</h3><p className="mt-1 text-xs text-[#8d5d55]">{error}</p></div><Button className="w-full sm:w-auto" variant="outline" size="sm" onClick={runAnalysis}><RefreshCw className="h-3.5 w-3.5" /> Try again</Button></div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <section className="overflow-hidden rounded-xl border bg-white">
        <div className="flex items-center gap-3 border-b bg-[#fafbf8] px-4 py-3.5 sm:px-5">
      ...
      {status === "complete" && analysis && (
        <div id="message-analysis-results" className="scroll-mt-6 space-y-4 page-enter">
          <section className="rounded-xl border border-[#efccc5] bg-[#fff8f6] p-4 sm:p-5">
          ...
          <section className="overflow-hidden rounded-xl border bg-white">
            ...
            <div className="flex flex-col gap-3 border-t bg-[#fafbf8] px-4 py-4 sm:flex-row sm:items-center sm:px-5"><div className="flex flex-1 items-center gap-2 text-[10px] text-[#77847c]"><ShieldCheck className="h-4 w-4 shrink-0 text-[#177553]" /> Sending remains disabled until a human approves this action.</div>{addedToApprovals ? <Button className="w-full sm:w-auto" asChild><Link href="/approvals"><CheckCircle2 className="h-4 w-4" /> View in approvals <ArrowRight className="h-3.5 w-3.5" /></Link></Button> : <Button className="w-full sm:w-auto" data-demo-target="add-to-approvals" onClick={addToApprovals} disabled={addingApproval}>{addingApproval ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <ClipboardCheck className="h-4 w-4" />} {addingApproval ? "Saving…" : "Add to approvals"}</Button>}</div>
