...
export default function CaseStudyPage() {
  return (
    <div className="page-enter space-y-8 pb-8">
      <section className="relative overflow-hidden rounded-2xl bg-[#132019] px-5 py-7 text-white sm:px-6 sm:py-8 md:px-10 md:py-12">
        ...
        <div className="relative max-w-4xl">
          ...
          <div className="mt-7 grid gap-3 sm:flex sm:flex-wrap"><Button className="w-full sm:w-auto" asChild><Link href="/inbox"><Sparkles className="h-4 w-4" /> Run flagship demo <ArrowRight className="h-3.5 w-3.5" /></Link></Button><Button variant="outline" className="w-full border-[#3d5347] bg-transparent text-white hover:bg-[#203329] hover:text-white sm:w-auto" asChild><Link href="/runs"><FlaskConical className="h-4 w-4" /> Inspect evals</Link></Button><Button variant="ghost" className="w-full text-[#bdd0c5] hover:bg-white/5 hover:text-white sm:w-auto" asChild><a href="https://github.com/AmalEN20/RetainAI" target="_blank" rel="noreferrer"><Code2 className="h-4 w-4" /> Source code <ExternalLink className="h-3 w-3" /></a></Button></div>
        </div>
      ...
      <section className="flex flex-col items-start justify-between gap-5 rounded-2xl border border-[#cfe6da] bg-[#edf8f2] p-5 sm:flex-row sm:items-center sm:p-6"><div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#177553]">Try the product</p><h2 className="mt-2 text-lg font-bold">See the complete workflow in under two minutes.</h2><p className="mt-1 text-xs text-[#68796f]">No account, API key, or paid service required.</p></div><Button className="w-full sm:w-auto" asChild><Link href="/inbox"><Sparkles className="h-4 w-4" /> Start with the cancellation email <ArrowRight className="h-3.5 w-3.5" /></Link></Button></section>
    </div>
  );
}
