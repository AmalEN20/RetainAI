...
import { resetDemoWorkspace } from "@/lib/demo/store";
...
export async function POST(request: Request) {
  ...
  if (!id) return NextResponse.json({ error: "Demo session not found." }, { status: 404 });
  return NextResponse.json(await resetDemoWorkspace(id), { headers: { "Cache-Control": "no-store" } });
}
